...
pglm=`psql -h $pghost -p $pgport -U $pguser -w -t -c 'SELECT NULL /*lm*/ FROM kladr_upd ORDER BY id DESC LIMIT 1' $pgbase | sed -e 's/^[ ]*//i'`
...
if [ "$rc" = "200" ] && [ "$lm" != "" ] && [ "$lm" != "$pglm" ]; then
  ...
  ls ${dir}/import_new/*.sql | xargs -l readlink -f | xargs -l -I{} cat {} >>${_dbf}/.sql
